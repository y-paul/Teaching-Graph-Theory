...
text_df <- text_df <- text_df %>% left_join(sentiments)
colnames(text_df)[9] <- "sentiment"



text_df %>% group_by(album) %>% summarize(m_emotional = mean(sentiment, na.rm=T),
                                          sd_emotional = sd(sentiment, na.rm=T))
...
word_pair <- word_pair %>% filter(Weight > 1)
...
# ready for gephi
write.csv(nodes_df, str_c("taylor_swift/data/","ts_","nodes.csv"), row.names=F, fileEncoding="utf-8")
write.csv(edges_df, str_c("taylor_swift/data/", "ts_","edges.csv"), row.names=F, fileEncoding="utf-8")
