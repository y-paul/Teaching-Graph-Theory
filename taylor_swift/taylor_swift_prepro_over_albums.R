...
process_album <- function(albums) {
  dat <- read_csv("taylor_swift/taylor_swift_lyrics.csv")
  
  dat <- dat %>% filter(album==albums)
  ...
  # ready for gephi
  write.csv(nodes_df, str_c("taylor_swift/data/albums/",albums,"_ts_","nodes.csv"), row.names=F, fileEncoding="utf-8")
  write.csv(edges_df, str_c("taylor_swift/data/albums/", albums,"_ts_","edges.csv"), row.names=F, fileEncoding="utf-8")
  
}
process_album("1989")
process_album("Fearless")
process_album("Red")
process_album("reputation")
process_album("Speak Now")
process_album("Taylor Swift")
