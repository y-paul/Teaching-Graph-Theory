...
gmaps_data_raw <- fromJSON("googlemaps/2020_April.json")
...
base_plot <- ggmap(map)
plot_2020_maps <- base_plot +   geom_line(data=pivot_data, aes(group=move_id, x=long_from, y= lat_from), alpha=.15, size=1.2) +
  geom_point(
    data=pivot_data,
    aes(x=long_from, y=lat_from),
    size=2,
...
igraph_2020 <- igraph_obj

co <- layout_with_fr(igraph_2020)
